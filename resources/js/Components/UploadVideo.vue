    ...
    <div class="flex flex-col w-full h-full">
        <div class="flex flex-row flex-wrap">
            <ul role="list" class="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-3 xl:gap-x-8">
                <li v-for="(video, index) in uploadedVideos" :key="index" class="relative" @mouseenter="showDeleteButton(index)" @mouseleave="hideDeleteButton(index)">
                    <div class="group aspect-h-7 aspect-w-10 block h-full w-full overflow-hidden rounded-lg bg-gray-100 focus-within:ring-2 focus-within:ring-green-500 focus-within:ring-offset-2 focus-within:ring-offset-gray-100">
                        <video :src="video.url" controls class="pointer-events-none object-cover group-hover:opacity-75 h-full"></video>
                    </div>
                    <button
                        v-show="video.showDeleteButton"
                ...
                <li class="relative">
                    <div class="group aspect-h-7 aspect-w-10 h-full block w-full">
                        <label class="h-40 min-w-[266px] flex flex-col items-center justify-center justify-items-center rounded border-2 border-zinc-500 dark:border-gray-100 hover:bg-gray-800 hover:bg-opacity-75 dark:hover:bg-gray-100 dark:hover:text-gray-900 dark:text-gray-100 hover:text-gray-200" for="file-upload">
                            ...
                            <input id="file-upload" class="sr-only" multiple name="file-upload" type="file" accept=".mp4" @change="handleVideoChange"/>
                        </label>
                    </div>
                </li>
            </ul>
...
export default {
    ...
    data() {
        return {
            uploadedVideos: this.videoList ? JSON.parse(this.videoList) : [],
            uploading: false,
            process: 0
        }
    },
    methods: {
            ...
            if (files) {
                for (let i = 0; i < files.length; i++) {
                    ...
                    this.calculateMD5(file)
                        .then((md5) => {
                            axios.post(this.checkUrl, { path: 'videos/' + md5 + '.' + extension })
                                .then(res => {
                                    if (res.data.code === 200) {
                                        ...
                                        this.postVideo(this.inputKey, JSON.stringify(this.uploadedVideos));
                                    } else {
        ...
        deleteVideo(index) {
            ...
            axios.patch(this.saveUrl, {
                [this.inputKey]: JSON.stringify(this.uploadedVideos)
            })
        },
        showDeleteButton(index) {
            this.uploadedVideos[index].showDeleteButton = true;
        },
        hideDeleteButton(index) {
            this.uploadedVideos[index].showDeleteButton = false;
        },
    },
};
</script>
