@props(['label', 'attribute', 'value', 'options'])
...
<div class="pt-3 sm:flex text-base" x-data="{
    ...
    <dt class="font-medium  sm:w-64 sm:flex-none sm:pr-6 leading-9">{{ $label }}</dt>
    <dd class="mt-1 flex justify-between gap-x-6 sm:mt-0 sm:flex-auto">
        <select id="{{ $attribute }}" x-on:change="toggle($event.target.value)" name="{{ $attribute }}" class="block bg-white bg-opacity-50 w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-400 focus:ring-opacity-50 disabled:opacity-50 dark:bg-zinc-700 dark:bg-opacity-80 dark:border-zinc-400 dark:focus:border-green-500 dark:focus:ring-green-500 dark:focus:ring-opacity-50 dark:text-gray-200">
            @foreach ($options as $option)
                <option value="{{ $option['code'] }}" {{ $option['code'] == $value ? 'selected' : '' }}>{{ $option['name'] }}</option>
            @endforeach
        </select>
    </dd>
