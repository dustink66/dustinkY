<tbody class="divide-y divide-gray-200 bg-white bg-opacity-50 dark:bg-zinc-900  dark:bg-opacity-50">
    @forelse($table->resource as $itemKey => $item)
        <tr
            :class="{
                'bg-gray-50': table.striped && @js($itemKey) % 2,
                'hover:bg-gray-100 dark:hover:bg-zinc-700': table.striped,
                'hover:bg-gray-50 dark:hover:bg-zinc-700': !table.striped
            }"
        >
            @if($hasBulkActions = $table->hasBulkActions())
                <td width="64" class="text-xs px-6 py-4">
                    ...
                    <input
                        ...
                        class="rounded border-gray-300 text-green-600 shadow-sm focus:border-green-300 focus:ring focus:ring-green-200 focus:ring-opacity-50 disabled:opacity-50"
            ...
            @foreach($table->columns() as $column)
                <td
                    ...
                    class="whitespace-nowrap text-base @if($loop->first && $hasBulkActions) pr-6 @else px-6 @endif py-4 @if($column->highlight) text-gray-900 dark:text-gray-100 font-medium @else text-gray-500 dark:text-gray-200 @endif @if($table->rowLinks->has($itemKey)) cursor-pointer @endif {{ $column->classes }}"
                >
        ...
        <tr>
            <td colspan="{{ $table->columns()->count() }}" class="whitespace-nowrap">
                @if(isset($emptyState) && !!$emptyState)
                    {{ $emptyState }}
                @else
                    <p class="text-gray-700 dark:text-gray-200 px-6 py-12 font-medium text-sm text-center">
